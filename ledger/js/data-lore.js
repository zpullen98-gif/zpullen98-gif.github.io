...
const GLOSSARY = [
  ...
  /* On Tap. Beer, cider, sake and mead each brought their own vocabulary,
     ...
     for. Nothing here duplicates a term above it: Beer gas already carries
     nitro and Half-barrel already carries the sixtel. */
  { term:"Wort", def:"The sweet unfermented liquid run off a mash, before yeast touches it. Its density is the original gravity, so the wort is where a beer's strength is settled and everything after it is a choice about what the yeast does with that." },
  { term:"Mash", def:"Milled malt held in hot water, 145 to 158F, while the malt's own enzymes cut starch into sugar. Hotter does not mean stronger: a cooler mash leaves less the yeast cannot eat, so it finishes drier and close to a percent stronger." },
  { term:"Attenuation", def:"The share of the wort's sugar the yeast ate, quoted apparent off a hydrometer. Ethanol is lighter than water, so the number flatters the beer and the real figure is about four fifths of it. Ales land from 65 to 85 percent." },
  { term:"OG and FG", def:"Original and final gravity: the wort before fermentation and the beer after. Subtract the second from the first and multiply by 131 for the alcohol. Ask a rep for the finishing number, because near 1.008 drinks bone dry and near 1.018 drinks full." },
  { term:"IBU", def:"International bitterness units, a lab reading of dissolved isomerised alpha acid. It measures what is in solution, not what the guest feels, which is why a 70 IBU hazy tastes softer than a 35 IBU pilsner." },
  { term:"SRM", def:"The colour scale: pilsner malt near 2, Munich 8 to 20, crystal 10 to 120, chocolate near 350, black 500 to 600. Colour is the maltster's heat, so it ranks neither enzyme power nor strength." },
  { term:"Adjunct", def:"Any fermentable other than malted barley: rice, corn, wheat, oats, sugar, honey, lactose. It is a recipe decision rather than a cheapening, which is why a Belgian tripel and an American light lager both lean on one." },
  { term:"Dry hop", def:"Hops added cold, after the boil, for aroma. Nothing isomerises, but humulinones still dissolve and bite at about two thirds the strength, so never promise a bitterness-shy guest that a heavily dry hopped beer pours soft." },
  { term:"Hop creep", def:"Hop enzymes freeing dextrins the yeast had quit on, so a dry hopped keg ferments again in your cooler. Alcohol climbs 0.3 to 1 percent, carbonation climbs with it, and the restarted ferment can leave butter behind. Warmth sets the clock." },
  { term:"DMS", def:"Dimethyl sulfide: creamed corn or cooked vegetable. The boil makes it and the steam carries it off, so a lidded kettle, a long hot stand or a slow chill leaves it in the beer. That one is the brewery's, not your cellar's." },
  { term:"Lactose", def:"Milk sugar, which brewer's yeast cannot ferment, added for sweetness and body in milk and pastry stouts. It is dairy, so it is the one common beer ingredient a vegan or a lactose-intolerant guest has to be told about." },
  { term:"Fining", def:"A dose that drags haze down so it settles out, not a filter. Two are animal: isinglass, off fish swim bladders, and gelatin. Irish moss, silica gel and PVPP are not. US labels list none of them, so check with the brewery rather than the glass." },
  { term:"Lambic and gueuze", def:"Lambic is spontaneously fermented Belgian wheat beer aged in old neutral barrels. Gueuze blends young lambic into old so the young sugar restarts it in the bottle. Neither takes oak flavour, because those staves went neutral decades ago." },
  { term:"Cask ale", def:"Unfiltered beer that finishes fermenting in the vessel it is served from, vented rather than sealed, pulled by a beer engine or by gravity at cellar temperature. It is alive and short-lived: days once broached, not weeks." },
  { term:"Growler and crowler", def:"Draught sold to go. A growler is a refillable jug, usually 32 or 64 oz; a crowler is a can seamed to order. Both are oxygen clocks measured in days, and a growler is only as good as its seal and its fill." },
  { term:"Session beer", def:"A beer built to be drunk more than once in a sitting, roughly under 5 percent. It is a claim about strength and not a style, so a session IPA is an IPA recipe brought down rather than a watered one." },
  { term:"Perry", def:"Fermented perry pears, and not pear cider. The fruit is close to inedible raw and carries sorbitol the yeast cannot eat, so a perry finished bone dry still tastes gently sweet. Pear cider usually means concentrate, or an apple base carrying pear flavour." },
  { term:"Keeving", def:"Starving a cider ferment on purpose. Calcium and pectin lift a brown cap that carries the juice's nitrogen off with it, so the yeast stalls with sugar still in the tank. That is how a Normandy cidre lands sweet and low in alcohol with nothing added." },
  { term:"Sorbitol", def:"A sugar alcohol in apples and, far more, in perry pears. Yeast cannot ferment it, so it survives as sweetness and body and it still reads on a hydrometer, which is why a perry measured dry does not taste dry." },
  { term:"Residual sugar", def:"The sugar left after fermentation, in grams per litre. No US rule attaches a number to dry, semi-sweet or sweet, so the word on the label promises nothing. Ask the rep for the figure, because it is what decides whether a cider comes back." },
  { term:"Bittersweet apple", def:"A cider apple low in acid and over roughly 0.2 percent tannin, the class West Country and Normandy cider is built on. It is not sweeter than the rest, it simply lacks the acid to taste sour, and it gets blended against sharp fruit for lift." },
  { term:"Mousiness", def:"A cider and natural wine fault from Brettanomyces or lactic bacteria. It is locked and odourless at drink pH, so there is nothing to nose; seconds after tasting, neutral saliva releases it as stale popcorn and mouse cage, and it builds rather than fades." },
  { term:"Koji", def:"Steamed rice grown through with the mould Aspergillus oryzae. It makes the amylase malt makes, but its enzymes carry on into the tank, releasing sugar while the yeast is already eating it, which is how sake reaches 18 to 20 percent." },
  { term:"Seimaibuai", def:"The polishing ratio: the share of each rice grain left, so 60 percent means 40 percent was milled away and a lower number means more is gone. It sets the legal grades and it costs money. It does not promise a better sake." },
  { term:"Junmai", def:"Sake of rice, koji and water with no distilled alcohol added. It is a statement about ingredients rather than a rank, and since 2004 it carries no milling requirement at all, so a junmai can be milled less than the honjozo standing beside it." },
  { term:"Ginjo and daiginjo", def:"Grades demanding a slow cold ferment and milling to 60 percent or less, and to 50 percent or less. Both are made for aromatics that heat drives off, so both are poured cold whatever else the label says." },
  { term:"Futsushu", def:"Ordinary sake: everything outside the eight special designations, and still most of what Japan drinks. A low legal bar is not a bad liquid, and this is what the guest who says they hate sake was served hot at a cheap table." },
  { term:"Jozo alcohol", def:"Neutral distilled spirit run into the sake mash just before pressing, capped in the special designations at 10 percent of the weight of the polished rice. It carries aroma into the sake instead of the lees and lightens the body. It is not fortification." },
  { term:"Nihonshu-do", def:"The sake meter value: a density reading against water, positive for lighter and usually drier, negative for denser and sweeter. It is not a sugar figure, and acidity changes how sweet the same number tastes, so it predicts less than it looks." },
  { term:"Namazake", def:"Unpasteurised sake, its koji enzymes still working in the bottle. It wants 32 to 41F from the moment it arrives and dies on a dry shelf: held warm it hazes, sours and turns soy-like, and chilling it afterwards undoes nothing." },
  { term:"Nigori", def:"Coarsely strained sake, white with rice solids in suspension. It is meant to be cloudy, so say so before you pour rather than after it comes back. Keep it cold, vent a lively one, and upend it slowly instead of shaking." },
  { term:"Genshu", def:"Undiluted sake. Most is watered back from 18 to 20 percent to the 15 or 16 on the label; a genshu is not, so it lands high and it is the one sake that takes a large cube without falling apart." },
  { term:"Atsukan and nurukan", def:"Warm sake service: nurukan near 104F, jokan near 113F, atsukan near 122F. Warming is a real technique and it suits junmai, kimoto and yamahai, while it boils off the aromatics a ginjo was made for." },
  { term:"Braggot, hydromel and melomel", def:"Mead names. Braggot is malt and honey together, hydromel is honey and water and also names the low strength class, melomel is fruit, with apples a cyser and grapes a pyment. Herbs or spice make a metheglin." },
  /* Coffee & Tea. Espresso, brewing, milk and the leaf each brought their
     own vocabulary, and a word the tab uses and never defines is what this
     list is for. Nothing here duplicates a term above it: the existing
     Oxidation entry is the wine and beer one, so the tea sense is named
     separately rather than overwritten. */
  { term:"Dose, yield and ratio", def:"The three numbers that define a shot: grams of dry coffee in, grams of liquid out, and the ratio between them. A modern double runs near 18 to 20 g in and roughly twice that out. Ratio is the language, because a shot is not a fixed volume." },
  { term:"Crema", def:"The foam on a fresh espresso. Carbon dioxide from roasting is driven into solution by the pump and comes back out as the liquid leaves the pressurised bed, held by coffee surfactants. It reads roast freshness and blend, not quality, and it does not survive a shake." },
  { term:"Over-extraction and under-extraction", def:"The two directions a shot misses. Under reads sour, thin and short, and the shot usually ran fast. Over reads drying and hollow at the finish. A guest calls both of them bitter, which is why you taste before you believe the complaint." },
  { term:"Tamp", def:"Compressing the bed of grounds so water cannot find a soft path. Level matters far more than force, and the trade no longer defends the old thirty pound figure. A tilted tamp leaves the leaning side thinner but denser, so water goes the other way." },
  { term:"Puck", def:"The compacted bed of grounds in the basket. Puck prep is everything done to it before the shot: breaking clumps through the whole depth rather than raking the top, levelling, then tamping." },
  { term:"Portafilter", def:"The handled basket that locks into the group. A naked or bottomless one has the spouts removed so you can watch the underside of the bed, which is the only way to see a channel form." },
  { term:"Channelling", def:"Water finding a fissure through the puck and taking it, leaving the rest barely touched. It is the path of least resistance, so the usual signature is a shot that runs fast, and it gives you sour and bitter in the same cup." },
  { term:"Burr grinder", def:"Two cutting surfaces set a fixed gap apart, flat or conical, as against blades that smash. Burrs heat and expand as a rush goes on, which narrows the gap and sends shots finer and slower." },
  { term:"Purge", def:"Running a little coffee through after a grind change, because the chute and burr chamber still hold the old setting. Skip it and the next dose is a blend of both, which is how people chase an adjustment the wrong way." },
  { term:"Backflush", def:"Running water back through the group against a blind basket, with detergent on the deep cycle, to clear coffee oil from the valve and screen. Rancid oil is the fault it prevents, and it tastes like the drink before last." },
  { term:"Ristretto and lungo", def:"The same dose at a shorter or longer yield. Ristretto means restricted, roughly 1:1 to 1:1.5. Lungo runs long, nearer 1:3 to 1:4. A lungo is not a watered espresso; it is more water through the same coffee, which is a different extraction." },
  { term:"Microfoam", def:"Steamed milk with bubbles too small to see, wet and glossy like paint rather than stiff. It is what pours a pattern. Froth is the failure: big dry bubbles that sit on top and arrive as a white blob." },
  { term:"Stretching and texturing", def:"The two halves of steaming. Stretching is the air phase, tip just under the surface, tearing air in while the milk is still cool. Texturing is the spin that follows, tip sunk, folding those bubbles smaller. Heat is the clock, not the job." },
  { term:"Rosetta and tulip", def:"The two patterns past a heart. A rosetta is poured with a wiggle and then dragged forward through the middle; a tulip is stacked in separate pulses and pushed together. Both are the heart with the pour broken up, and both take months." },
  { term:"Immersion and percolation", def:"The two brewing families. Immersion steeps the grounds in the whole volume then separates them. Percolation passes water through a bed and away. Hold everything else equal and percolation extracts more, but body and clarity come from the FILTER, not the family." },
  { term:"Bloom", def:"The first small pour on a pour-over, which wets the bed and lets roast carbon dioxide escape before the real pours begin. Skip it and the gas pushes water around rather than through." },
  { term:"Cold brew and flash brew", def:"Two ways to a cold coffee. Cold brew never sees heat, steeping coarse grounds for many hours. Flash brew, or Japanese iced, brews hot straight onto ice that counts as part of the brew water. They taste nothing alike and only one is made to order." },
  { term:"Nitro cold brew", def:"Cold brew charged with nitrogen and poured through a restrictor plate. Nitrogen is barely soluble and forms no carbonic acid, so it adds no flavour and no prickle; the cascade and dense head are mechanical. Usually straight nitrogen, not the beer gas blend." },
  { term:"Hardness and alkalinity", def:"Two separate things in water that are often confused because they travel together. Hardness is calcium and magnesium. Alkalinity is carbonate buffering, and it is the one that flattens acidity and mutes a cup. They are measured separately and they fault in different directions." },
  { term:"Scale", def:"Mineral deposited out of hard water by heat, inside boilers, lines and heat exchangers. It insulates, it restricts, and it eventually kills the machine. It is not the same fault as rancid coffee oil, and the two taste nothing alike." },
  { term:"Tea oxidation", def:"The enzymatic browning that separates white, green, oolong and black tea, which all come off one plant. It is NOT fermentation, whatever a tea list says: bruising the leaf brings an enzyme into contact with its own compounds and the oxidant is air." },
  { term:"Tisane", def:"An infusion of something that is not the tea plant: chamomile, peppermint, rooibos, hibiscus. The word matters for two reasons, caffeine and allergies. Most tisanes carry no caffeine, though yerba mate and guayusa do, so it is not a rule you can state as a definition." },
  { term:"Pu-erh", def:"A dark tea from the hei cha family, and the real exception to oxidation, because it genuinely is fermented by microbes. Sheng ages slowly; shou is pushed through a wet piling that compresses decades into months." },
  { term:"Orange pekoe", def:"A leaf grade, not a flavour, a type or a mark of quality. It describes whole leaf of a certain size, and the longer strings of letters above it describe size and tip content rather than how the tea tastes. A guest ordering one usually means plain black tea." },
  { term:"Gongfu and the gaiwan", def:"A Chinese way of brewing: a lot of leaf in a small vessel for very short steeps, poured off again and again from a lidded bowl called a gaiwan. It is the clearest demonstration of what a teabag cannot do." },
  { term:"Matcha, usucha and the chasen", def:"Shade grown green tea stone ground to powder and whisked into suspension rather than steeped, so you drink the leaf. Usucha is the ordinary thin preparation; the chasen is the bamboo whisk. Sift it first or it clumps, and move the whisk in a W rather than a circle." },
  { term:"Kill-green", def:"The heat step that stops oxidation dead, by steaming or pan firing, and the reason a green tea stays green. Japanese and Chinese greens taste different largely because one steams and the other pans." },
  { term:"Masala chai", def:"Spiced milk tea, and a recipe rather than an ingredient. Chai just means tea, so chai tea says tea twice. The spices need to simmer in the milk rather than steep in water, which is the step a bar tries to skip." },
  { term:"Americano and long black", def:"Espresso and hot water, and the order is the whole difference. Water into espresso drowns the crema; espresso into water floats it. A long black is the second one, and it is also a cocktail name in this ledger, so ask which the guest means." },
  { term:"Decaf", def:"Coffee with most of its caffeine removed, by water, solvent or carbon dioxide. Most is left is the wrong assumption in both directions: a decaf still carries a little, and the bean behaves differently in the grinder, so it needs its own dial-in rather than the last one." },
];
